...
const LandingPage2 = () => {
  return (
    <div className="relative w-full min-h-screen bg-primary flex flex-col items-center gap-20">
      {/* Decorative Circles */}
      <Circle
        size="40px"
        color="#FFBA00"
        position="top-50 left-10 animate-bounce-slow"
      />
      <Circle
        size="27px"
        color="#ED4883"
        className="laptop:top-60 right-30 tablet:top-60 mobile:top-200 mobile:right-5 animate-bounce-slow"
      />
      ...
      <div className="flex flex-col items-center justify-center text-center mt-20">
        <h1 className="font-rubik text-[clamp(24px,5vw,42px)] font-semibold text-[#040428] leading-snug">
          KS2 All-Through Reporting
        </h1>
        <p className="font-rubik text-[clamp(16px,2vw,20px)]  font-normal text-[#504D4E] max-w-[555px]">
      ...
      {/* Card Section */}
      <div className="grid tablet:grid-cols-2 mobile:grid-cols-1  laptop:grid-cols-4 gap-8 w-[90%] mx-auto">
        {cardData?.map((card, index) => (
          <div
            key={index}
            className="flex flex-col items-start justify-between bg-white rounded-[24px] shadow-md hover:shadow-lg transition-all duration-300 w-full min-h-[360px]"
          >
            {/* Image Section */}
            <div
            ...
            <div className="flex flex-col justify-start items-start px-[34px] flex-1">
              <h3 className="font-rubik text-[clamp(18px,3vw,22px)] font-semibold text-[#2F2E51] mb-2">
                {card.title}
              </h3>
              <p className="text-[#504D4E] font-rubik font-normal text-[clamp(14px,2.5vw,16px)] leading-8">
                {card.text}
              </p>
            </div>
          </div>
